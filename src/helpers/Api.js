...
export function generateReport(files) {
  return new Promise((resolve, reject) => {
    const fileDataArray = [];
    if (files && files.length > 0) {
      for (let i = 0; i < files.length; i++) {
        ...
        reader.onload = function (event) {
          ...
          if (fileDataArray.length === files.length) {
            axios({
              method: "post",
              url: "https://chestx.localto.net/reportgen",
              data: fileDataArray,
            })
              .then((res) => {
                resolve(res.data.results[0].generated_caption); // Resolve with the generated caption
                // console.log(res.data.results[0].generated_caption);
              })
              .catch((err) => {
                reject(err); // Reject with the error
              });
